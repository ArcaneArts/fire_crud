...
extension XFCrudRoot$Task on RootFireCrud {
  ...
  Future<ModelPage<Task>?> paginateTasks({int pageSize = 50, bool reversed = false, CollectionReference Function(CollectionReference ref)? query}) => paginate<Task>(pageSize: pageSize, reversed: reversed, query: query);
  
  /// Deletes all [Task] in the collection optionally filtered by [query]
  Future<void> deleteAllTasks([CollectionReference Function(CollectionReference ref)? query]) => deleteAll<Task>(query);
