...
extension XFCrudBase$Task on Task {
  ...
  /// Modifies properties of the [Task] document (as a unique child) atomically.
  Future<void> modify({
...
extension XFCrud$Task$Subtask on Task {
  ...
  Future<ModelPage<Subtask>?> paginateSubtasks({
    int pageSize = 50,
    bool reversed = false,
    CollectionReference Function(CollectionReference ref)? query,
  }) => paginate<Subtask>(pageSize: pageSize, reversed: reversed, query: query);
...
extension XFCrudBase$Subtask on Subtask {
  ...
  /// Modifies properties of the [Subtask] document (as a unique child) atomically.
  Future<void> modify({
    /// Replaces the value of [title] with a new value atomically.
    String? title,
...
extension XFCrudRoot$Task on RootFireCrud {
  ...
  /// Gets a document page starting at the beginning of the collection. Returns a [ModelPage<Task>] that contains the models and nextPage method to get the next page
  Future<ModelPage<Task>?> paginateTasks({
    int pageSize = 50,
    bool reversed = false,
    CollectionReference Function(CollectionReference ref)? query,
  }) => paginate<Task>(pageSize: pageSize, reversed: reversed, query: query);

  /// Deletes all [Task] in the collection optionally filtered by [query]
  Future<void> deleteAllTasks([
    CollectionReference Function(CollectionReference ref)? query,
  ]) => deleteAll<Task>(query);

  /// Opens a stream of all [Task] in the collection optionally filtered by [query]
  Stream<List<Task>> streamTasks([
    CollectionReference Function(CollectionReference ref)? query,
  ]) => streamAll<Task>(query);
  ...
  Future<void> setTaskAtomic(String id, Task Function(Task?) txn) =>
      $setAtomic<Task>(id, txn);

  /// Ensures that the [Task] document with [id] exists, if not it will be created with [or]
  Future<void> ensureTaskExists(String id, Task or) =>
      $ensureExists<Task>(id, or);
